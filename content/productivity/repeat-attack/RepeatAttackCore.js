...
function pesterS(
    ...
    const TRIGGER_AMOUNT = Math.floor(
      (source.silverCap * percentageTrigger) / 100
    );
    const FUZZY_ENERGY = Math.floor(source.silver - unconfirmedDepartures);
    ...
    const match2 = df
      .getMyPlanets()
      .filter((t) => t.locationId == opponentsPlanetLocationsId);
    if (match2.length == 0) {
      return;
    }
    ...
    const TRIGGER_AMOUNT2 = Math.floor(
        (to.silverCap * percentageTrigger) / 100
      );
